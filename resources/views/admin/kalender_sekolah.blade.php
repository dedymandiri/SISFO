...
<div>
  ...
  <tbody>
    ...
    @foreach($liat as $li)
    <tr>
        <td>{{ $li->id_kalender}}</td>
        <td>{{ $li->keterangan }}</td>
        <td><embed src="{{ url('/data_kalender/'.$li->file) }}" type="application/pdf" width="100%" height="600px"></td>
               
        <td>
            <a href="/edit_kalender_sekolah/{{ $li->id_kalender }}"><button style="width:65px;" title="Edit" type="button"  class="btn btn-warning">Edit</button></a>
        
            <a href="/kalender_sekolah/hapus/{{ $li->id_kalender }}"><button onclick="return confirm('Apakah anda yakin hapus data?')" title="Hapus" type="button" class="btn btn-danger">Hapus</button></a>
        </td>  
